...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://imgur.com/9PUeonC.jpg'
                    let ppgp = 'https://imgur.com/9PUeonC.jpg'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('KeNy_Bot', '/api/welcome', {
                                username: await this.getName(user),
                                groupname: await this.getName(id),
                                groupicon: ppgp,
                                membercount: groupMetadata.participants.length,
                                profile: pp,
                                background: 'https://imgur.com/q1WOvP0.jpg'
                            }, 'apikey')

                            let lea = API('KeNy_Bot', '/api/goodbye2', {
                                username: await this.getName(user),
                                groupname: await this.getName(id),
                                groupicon: ppgp,
                                membercount: groupMetadata.participants.length,
                                profile: pp,
                                background: 'https://imgur.com/r8rfCqe.jpg'
                            }, 'apikey')
        ...
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://imgur.com/9PUeonC.jpg') 
            if (!text)
